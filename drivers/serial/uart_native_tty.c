...
#define DT_DRV_COMPAT etc_native_tty_uart
...
DT_INST_FOREACH_STATUS_OKAY(NATIVE_TTY_RX_DEF);
...
#define NATIVE_TTY_CASE(inst) \
		...
		k_thread_name_set(tid, "native_tty_rx_"#inst ); \
		break;


static void native_tty_irq_initx(const struct device *dev,int inst)
{
	...
	switch(inst)
	{
		...
		DT_INST_FOREACH_STATUS_OKAY(NATIVE_TTY_CASE);
	}
}
...
static int native_tty_serial_initx(const struct device *dev, int inst)
{
	struct native_tty_data *data = dev->data;
	...
	native_tty_irq_initx(dev,inst);
#endif
	return 0;
}

#define NATIVE_TTY_INIT(inst) \
static int native_tty_serial_init_##inst (const struct device *dev) { \
	return native_tty_serial_initx(dev, (inst) ); \
} ; \

DT_INST_FOREACH_STATUS_OKAY(NATIVE_TTY_INIT);
...
#define NATIVE_TTY_INSTANCE(inst)                                                                  \
	...
	static struct native_tty_data native_tty_##inst##_data = {                                 \
		.serial_port = DT_INST_PROP_OR(inst, serial_port, NULL),                           \
	                                                                                                  ...
	                                                                                                  \
	DEVICE_DT_INST_DEFINE(inst, native_tty_serial_init_##inst , NULL, &native_tty_##inst##_data,       \
